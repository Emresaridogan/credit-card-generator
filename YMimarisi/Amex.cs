...
namespace YMimarisi
{
    public class Amex : KrediKarti
    {
        ...
        private String ccv;
        private Object logo = Properties.Resources.amex1;
        private double limit;
        private double puan;
        public Amex(string kartSahibi, string kartTuru, double limit, double puan)
        {
        ...
        public String kartNumarasınıGetir()
        {
            ...
            k2 = kart.Substring(4, 6);
            k3 = kart.Substring(10, 5);
            // **** ****** ***** şeklinde gözükmesi için 
            return k1 + " " + k2 + " " + k3;
        }
        ...
        public String sonKullanmaTarihiOlustur()
        {
            ...
            int sayi = rnd.Next(4, 9); // kart kullanım süresi 4 ile 9 yıl arası
            String dateYear = DateTime.Now.Year.ToString(); // şuanki yıl
            int yil = int.Parse(dateYear.Substring(2, 2)); // yılın son 2 rakamını alma
            yil = yil + sayi; // ccv yılı oluşturma
            int dateMonth = rnd.Next(1, 12); // ccv ayı oluşturma
        ...
        String kartNumarasiOlustur()
        {
            bool don = true; // algoritmanın kontrolü
            String kartNo = "";
            List<int> dizi = new List<int>(); // Amex kartının 2.rakamı 4 veya 7 olmalı
            dizi.Add(4); 
            dizi.Add(7);
            List<int> sayilar = new List<int>();
            Random rand = new Random();
            int sayi2 = dizi[rand.Next(0, 2)]; // rastgele 4 yada 7 gelecek
            while (don) // luhn algoritması doğru olduğu sürece dönecek
            {
                sayilar.Clear();
                int topla = 0;
                int gecici = 0;
                sayilar.Add(3); // amex numarası 3 ile başlar
                sayilar.Add(sayi2); // 2.rakam eklenir
                for (int i = 1; i <= 13; i++)
                {
